...
`cuda_configure` depends on the following environment variables:
  ...
  * `TACHYON_CUDA_PATHS`: The base paths to look for CUDA and cuDNN. Default is
    `/usr/local/cuda,usr/`.
  ...
  * `TACHYON_CUDNN_VERSION`: The version of the cuDNN library.
...
_TACHYON_CUDNN_VERSION = "TACHYON_CUDNN_VERSION"
...
def _cuda_autoconf_impl(repository_ctx):
    ...
    if not enable_cuda(repository_ctx):
        ...
        has_cudnn_version = get_host_environ(repository_ctx, _TACHYON_CUDNN_VERSION) != None
        if not has_cuda_version or not has_cudnn_version:
            auto_configure_fail("%s and %s must also be set if %s is specified" %
                                (_TACHYON_CUDA_VERSION, _TACHYON_CUDNN_VERSION, _TACHYON_CUDA_CONFIG_REPO))
