...
// clang-format off
#include "benchmark/poseidon/poseidon_benchmark_config.h"
...
int RealMain(int argc, char** argv) {
  PoseidonBenchmarkConfig config;
  ...
  PoseidonBenchmarkRunner<Field> runner(&reporter, config.repeating_num(),
                                        config.absorbing_num(),
                                        config.squeezing_num());
  runner.Run();
  runner.RunExternal(run_poseidon_arkworks);
