...
class ProjectivePointTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    cc_a_ = bn254::G1ProjectivePoint(c::math::ToCProjectivePoint(a_));
    cc_b_ = bn254::G1ProjectivePoint(c::math::ToCProjectivePoint(b_));
  }
...
TEST_F(ProjectivePointTest, Zero) {
  ...
  EXPECT_TRUE(c::math::ToProjectivePoint(c_ret.ToCPoint()).IsZero());
}

TEST_F(ProjectivePointTest, Generator) {
  bn254::G1ProjectivePoint c_ret = bn254::G1ProjectivePoint::Generator();
  EXPECT_EQ(c::math::ToProjectivePoint(c_ret.ToCPoint()),
...
TEST_F(ProjectivePointTest, Random) {
  ...
  EXPECT_NE(c::math::ToProjectivePoint(c_ret.ToCPoint()), a_);
}
...
TEST_F(ProjectivePointTest, Add) {
  ...
  EXPECT_EQ(c::math::ToProjectivePoint(cc_ret.ToCPoint()), a_ + b_);
  EXPECT_EQ(c::math::ToProjectivePoint((cc_a_ += cc_b_).ToCPoint()), a_ += b_);
}
...
TEST_F(ProjectivePointTest, Neg) {
  ...
  EXPECT_EQ(c::math::ToProjectivePoint(cc_ret.ToCPoint()), -a_);
}

TEST_F(ProjectivePointTest, Dbl) {
  bn254::G1ProjectivePoint cc_ret = cc_a_.Double();
  EXPECT_EQ(c::math::ToProjectivePoint(cc_ret.ToCPoint()), a_.Double());
}
