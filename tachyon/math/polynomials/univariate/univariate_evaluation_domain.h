...
class UnivariateEvaluationDomain : public EvaluationDomain<F, MaxDegree> {
 public:
  ...
  constexpr std::vector<F, base::memory::ReusingAllocator<F>> GetRootsOfUnity(
      size_t size, const F& root) const {
  ...
  EvaluateAllLagrangeCoefficients(const F& tau) const {
  ...
  constexpr std::vector<F, base::memory::ReusingAllocator<F>>
  EvaluatePartialLagrangeCoefficients(
    ...
    if (z_h_at_tau.IsZero()) {
      ...
      std::vector<F, base::memory::ReusingAllocator<F>> u(size, F::Zero());
      ...
      std::vector<F, base::memory::ReusingAllocator<F>>
          lagrange_coefficients_inverse(size);
      base::Parallelize(
  ...
  CONSTEXPR_IF_NOT_OPENMP std::vector<F, base::memory::ReusingAllocator<F>>
  GetElements() const {
