...
class Radix2EvaluationDomain : public UnivariateEvaluationDomain<F, MaxDegree> {
 ...
 private:
  ...
  // And here are example where |this->size_| equals to 32.
  // |root_vec_| = [
  //   [1],
  //   [1, ω⁸],
  //   [1, ω⁴, ω⁸, ω¹²],
  ...
  constexpr void PrepareRootsVecCache() {
    ...
    // Assign every elements based biggest vectors.
  ...
  constexpr void InOutHelper(DensePoly& poly) const {
    ...
    size_t idx = 0;
    while (gap > 0) {
      ApplyButterfly<FFTOrder::kInOut>(poly, inv_roots_vec_[idx], 1, gap);
      gap /= 2;
      idx += 1;
    }
  }
  ...
  constexpr void OutInHelper(Evals& evals, size_t start_gap) const {
    ...
    while (gap < evals.evaluations_.size()) {
      ApplyButterfly<FFTOrder::kOutIn>(evals, roots_vec_[idx], 1, gap);
      gap *= 2;
      idx += 1;
    }
  }
