...
int GenerationConfig::GenerateConfigHdr() const {
  std::vector<std::string_view> tpl = {
      ...
      "    if constexpr (BaseField::N > ScalarField::N) {",
      "      return ScalarField::FromBigInt((v.ToBigInt() % ScalarField::Config::kModulus.template Extend<BaseField::N>()).template Shrink<ScalarField::N>());",
      "    } else if constexpr (BaseField::N < ScalarField::N) {",
      "      return ScalarField::FromBigInt(v.ToBigInt().template Extend<ScalarField::N>() % ScalarField::Config::kModulus);",
      "    } else {",
      ...
      "    if constexpr (BaseField::N > ScalarField::N) {",
      "      return BaseField::FromBigInt(v.ToBigInt().template Extend<BaseField::N>() % BaseField::Config::kModulus);",
      "    } else if constexpr (BaseField::N < ScalarField::N) {",
      "      return BaseField::FromBigInt((v.ToBigInt() % BaseField::Config::kModulus.template Extend<ScalarField::N>()).template Shrink<BaseField::N>());",
      "    } else {",
