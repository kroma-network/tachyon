...
template __global__ void InitializeBucketsKernel<bn254::G1CurveGpu>(
    PointXYZZ<bn254::G1CurveGpu>* buckets, unsigned int count);

template gpuError_t InitializeBuckets<bn254::G1CurveGpu>(
    PointXYZZ<bn254::G1CurveGpu>* buckets, unsigned int count,
    gpuStream_t stream);

template __global__ void ComputeBucketIndexesKernel<bn254::FrGpu>(
...
template gpuError_t ComputeBucketIndexes<bn254::FrGpu>(
...
template __global__ void AggregateBucketsKernel<bn254::G1CurveGpu, false>(
...
template __global__ void AggregateBucketsKernel<bn254::G1CurveGpu, true>(
    const unsigned int* __restrict__ base_indexes,
    const unsigned int* __restrict__ bucket_run_offsets,
    const unsigned int* __restrict__ bucket_run_lengths,
    const unsigned int* __restrict__ bucket_indexes,
...
template gpuError_t AggregateBuckets<bn254::G1CurveGpu>(
...
template __global__ void ExtractTopBucketsKernel<bn254::G1CurveGpu>(
...
template gpuError_t ExtractTopBuckets<bn254::G1CurveGpu>(
    PointXYZZ<bn254::G1CurveGpu>* buckets,
    PointXYZZ<bn254::G1CurveGpu>* top_buckets, unsigned int bits_count,
    unsigned int windows_count, gpuStream_t stream);

template __global__ void SplitWindowsKernel<bn254::G1CurveGpu>(
...
template gpuError_t SplitWindows<bn254::G1CurveGpu>(
    unsigned int source_window_bits_count, unsigned int source_windows_count,
    const PointXYZZ<bn254::G1CurveGpu>* source_buckets,
    PointXYZZ<bn254::G1CurveGpu>* target_buckets, unsigned int count,
    gpuStream_t stream);

template __global__ void ReduceBucketsKernel<bn254::G1CurveGpu>(
    PointXYZZ<bn254::G1CurveGpu>* buckets, unsigned int count);

template gpuError_t ReduceBuckets<bn254::G1CurveGpu>(
    PointXYZZ<bn254::G1CurveGpu>* buckets, unsigned int count,
    gpuStream_t stream);

template __global__ void LastPassGatherKernel<bn254::G1CurveGpu>(
...
template gpuError_t LastPassGather<bn254::G1CurveGpu>(
    unsigned int bits_count_pass_one,
