...
struct Fibonacci1Config {
  ...
  Fibonacci1Config(std::array<AdviceColumnKey, 3>&& advice,
                   const Selector& selector, const InstanceColumnKey& instance)
      : advice(std::move(advice)), selector(selector), instance(instance) {}

  Fibonacci1Config Clone() const {
    std::array<AdviceColumnKey, 3> advice_clone = advice;
    return Fibonacci1Config(std::move(advice_clone), selector, instance);
  }
