...
class Fibonacci4CircuitTest : public CircuitTest {};

}  // namespace

TEST_F(Fibonacci4CircuitTest, CreateProof) {
  size_t n = 2048;
  CHECK(prover_->pcs().UnsafeSetup(n, F(2)));
  prover_->set_domain(Domain::Create(n));

  Fibonacci4Circuit<F, SimpleFloorPlanner> circuit;
  std::vector<Fibonacci4Circuit<F, SimpleFloorPlanner>> circuits = {
  ...
  std::vector<uint8_t> expected_proof(std::begin(fibonacci4::kExpectedProof),
                                      std::end(fibonacci4::kExpectedProof));
  EXPECT_THAT(proof, testing::ContainerEq(expected_proof));
}

TEST_F(Fibonacci4CircuitTest, VerifyProof) {
  size_t n = 2048;
  CHECK(prover_->pcs().UnsafeSetup(n, F(2)));
  prover_->set_domain(Domain::Create(n));
  ...
  std::vector<uint8_t> owned_proof(std::begin(fibonacci4::kExpectedProof),
                                   std::end(fibonacci4::kExpectedProof));
