...
TEST_F(SimpleLookupCircuitTest, Configure) {
  ...
  EXPECT_EQ(simple_lookup::kPinnedConstraintSystem,
...
TEST_F(SimpleLookupCircuitTest, LoadVerifyingKey) {
  ...
  EXPECT_EQ(simple_lookup::kPinnedVerifyingKey,
...
TEST_F(SimpleLookupCircuitTest, CreateProof) {
  ...
  std::vector<uint8_t> expected_proof(std::begin(simple_lookup::kExpectedProof),
                                      std::end(simple_lookup::kExpectedProof));
...
TEST_F(SimpleLookupCircuitTest, VerifyProof) {
  ...
  std::vector<uint8_t> owned_proof(std::begin(simple_lookup::kExpectedProof),
                                   std::end(simple_lookup::kExpectedProof));
