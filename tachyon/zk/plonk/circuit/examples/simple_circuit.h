...
class FieldConfig {
 public:
  ...
  FieldConfig() = default;
  FieldConfig(const std::array<AdviceColumn, 2>& advice,
              const InstanceColumn& instance, const Selector& s_mul)
      : advice_(advice), instance_(instance), s_mul_(s_mul) {}
...
class FieldChip {
 public:
  FieldChip() = default;
  explicit FieldChip(const FieldConfig<F>& config) : config_(config) {}

  static FieldConfig<F> Configure(ConstraintSystem<F>& meta,
                                  const std::array<AdviceColumn, 2>& advice,
                                  const InstanceColumn& instance,
                                  const FixedColumn& constant) {
    ...
    meta.CreateGate("mul", [&meta, &advice, &sel]() {
      ...
      return std::vector<Constraint<F>>({s_mul * (lhs * rhs - out)});
    });

    return FieldConfig<F>(advice, instance, sel);
  }
...
class SimpleCircuit : public Circuit<FieldConfig<F>> {
 public:
  ...
  static FieldConfig<F> Configure(ConstraintSystem<F>& meta) {
    ...
    return FieldChip<F>::Configure(meta, advice, instance, constant);
  }

  Error Synthesize(const FieldConfig<F>& config,
                   Layouter<F>* layouter) override {
    FieldChip<F> field_chip(config);
