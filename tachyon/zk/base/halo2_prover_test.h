...
class Halo2ProverTest : public testing::Test {
 public:
  ...
  constexpr static size_t kExtendedMaxDegree = (size_t{1} << 7) - 1;
  constexpr static size_t kExtendedDomainSize = kExtendedMaxDegree + 1;
  ...
  using ExtendedDomain =
      math::UnivariateEvaluationDomain<F, kExtendedMaxDegree>;
  ...
  void SetUp() override {
    ...
    std::unique_ptr<ExtendedDomain> extended_domain =
        math::UnivariateEvaluationDomainFactory<F, kExtendedMaxDegree>::Create(
            kExtendedDomainSize);
    ...
    prover_ = std::make_unique<Halo2Prover<PCS, ExtendedDomain>>(
        Halo2Prover<PCS, ExtendedDomain>::CreateFromRandomSeed(
            std::move(pcs), std::move(domain), std::move(extended_domain),
            std::move(writer), kBlindingFactors));
  }

 protected:
  std::unique_ptr<Halo2Prover<PCS, ExtendedDomain>> prover_;
};
