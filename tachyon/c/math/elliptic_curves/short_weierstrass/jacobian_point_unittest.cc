...
class JacobianPointTest : public testing::Test {
 public:
  void SetUp() override {
    ...
    g1_c_a_ = c::math::ToCJacobianPoint(g1_a_);
    g1_c_b_ = c::math::ToCJacobianPoint(g1_b_);

    g2_a_ = math::bn254::G2JacobianPoint::Random();
    g2_b_ = math::bn254::G2JacobianPoint::Random();
    g2_c_a_ = c::math::ToCJacobianPoint(g2_a_);
    g2_c_b_ = c::math::ToCJacobianPoint(g2_b_);
  }
...
TEST_F(JacobianPointTest, Zero) {
  ...
  EXPECT_TRUE(c::math::ToJacobianPoint(c_ret2).IsZero());
}
...
TEST_F(JacobianPointTest, Generator) {
  ...
  EXPECT_EQ(c::math::ToJacobianPoint(c_ret2),
...
TEST_F(JacobianPointTest, Random) {
  ...
  EXPECT_NE(c::math::ToJacobianPoint(c_ret), g1_a_);

  tachyon_bn254_g2_jacobian c_ret2 = tachyon_bn254_g2_jacobian_random();
  EXPECT_NE(c::math::ToJacobianPoint(c_ret2), g2_a_);
}
...
TEST_F(JacobianPointTest, Add) {
  ...
  EXPECT_EQ(c::math::ToJacobianPoint(c_ret), g1_a_ + g1_b_);
  ...
  EXPECT_EQ(c::math::ToJacobianPoint(c_ret), g1_a_ + d);

  tachyon_bn254_g2_jacobian c_ret2 =
      tachyon_bn254_g2_jacobian_add(&g2_c_a_, &g2_c_b_);
  EXPECT_EQ(c::math::ToJacobianPoint(c_ret2), g2_a_ + g2_b_);
  ...
  EXPECT_EQ(c::math::ToJacobianPoint(c_ret2), g2_a_ + d2);
}
