...
class ProjectivePointTest : public c::math::bn254::G1Test {
 public:
  void SetUp() override {
    a_ = math::bn254::G1ProjectivePoint::Random();
    b_ = math::bn254::G1ProjectivePoint::Random();

    c_a_ = c::math::ToCProjectivePoint(a_);
    c_b_ = c::math::ToCProjectivePoint(b_);
  }
...
TEST_F(ProjectivePointTest, Random) {
  ...
  EXPECT_NE(c::math::ToProjectivePoint(c_ret), a_);
}
...
TEST_F(ProjectivePointTest, Add) {
  ...
  EXPECT_EQ(c::math::ToProjectivePoint(c_ret), a_ + b_);
  ...
  EXPECT_EQ(c::math::ToProjectivePoint(c_ret), a_ + d);
}
