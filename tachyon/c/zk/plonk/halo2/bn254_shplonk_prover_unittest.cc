...
TEST_P(SHPlonkProverTest, Getters) {
  ...
  EXPECT_EQ(tachyon_halo2_bn254_shplonk_prover_get_blinder(prover_),
            c::base::c_cast(&(c::base::native_cast(prover_)->blinder())));
  EXPECT_EQ(tachyon_halo2_bn254_shplonk_prover_get_domain(prover_),
            c::base::c_cast(c::base::native_cast(prover_)->domain()));
...
TEST_P(SHPlonkProverTest, Commit) {
  ...
  tachyon_bn254_g1_jacobian* point = tachyon_halo2_bn254_shplonk_prover_commit(
      prover_, c::base::c_cast(&poly));
  EXPECT_EQ((c::base::native_cast(prover_)->Commit(poly).ToJacobian()),
...
TEST_P(SHPlonkProverTest, CommitLagrange) {
  ...
  EXPECT_EQ((c::base::native_cast(prover_)->Commit(evals).ToJacobian()),
            c::base::native_cast(*point));
}
...
TEST_P(SHPlonkProverTest, SetRng) {
  ...
  EXPECT_EQ((c::base::native_cast(prover_)->blinder().Generate()),
...
TEST_P(SHPlonkProverTest, SetTranscript) {
  ...
  EXPECT_EQ(
      (c::base::native_cast(prover_)->transcript()->SqueezeChallenge()),
