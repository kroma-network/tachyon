...
using PKeyImpl = c::zk::plonk::ProvingKeyImpl<LS>;
...
#define INVOKE(Method, ...)                                             \
  switch (static_cast<zk::plonk::halo2::LSType>(pk->ls_type)) {         \
    case zk::plonk::halo2::LSType::kHalo2: {                            \
      return Method(reinterpret_cast<PKeyImpl<Halo2LS>*>(pk->extra),    \
                    ##__VA_ARGS__);                                     \
    }                                                                   \
    case zk::plonk::halo2::LSType::kLogDerivativeHalo2: {               \
      return Method(                                                    \
          reinterpret_cast<PKeyImpl<LogDerivativeHalo2LS>*>(pk->extra), \
          ##__VA_ARGS__);                                               \
    }                                                                   \
  }                                                                     \
  NOTREACHED()
...
tachyon_bn254_plonk_proving_key_create_from_state(uint8_t ls_type,
  ...
  switch (static_cast<zk::plonk::halo2::LSType>(ls_type)) {
    case zk::plonk::halo2::LSType::kHalo2: {
      pkey->extra =
          new PKeyImpl<Halo2LS>(absl::Span<const uint8_t>(state, state_len),
                                /*read_only_vk=*/false);
      return pkey;
    }
    case zk::plonk::halo2::LSType::kLogDerivativeHalo2: {
      pkey->extra = new PKeyImpl<LogDerivativeHalo2LS>(
